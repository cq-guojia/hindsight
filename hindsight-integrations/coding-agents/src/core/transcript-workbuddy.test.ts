...
describe("readWorkbuddyTranscript", () => {
  it("renders a prompt, its tool call and the reply in log order", () => {
    ...
    const turns = read([
      ...
      { type: "function_call", name: "Edit", arguments: '{"file_path":"src/app.ts"}', timestamp: at("2026-08-14T10:00:02Z") },
      // tool OUTPUT — dropped, like every other reader keeps raw results out of the bank
      { type: "function_call_result", name: "Edit", output: { type: "text", text: "ok" }, timestamp: at("2026-08-14T10:00:02Z") },
    ...
    expect(turns).toEqual([
      ...
      { role: "assistant", content: "Patched the retry loop.", timestamp: "2026-08-14T10:00:03.000Z" },
    ]);
  });

  it("drops the host's own user records: compaction summaries and background-task notices", () => {
    const turns = read([
      userMsg("<cb_summary>\nSummary of the conversation so far: …\n</cb_summary>", at("2026-08-14T10:00:00Z")),
      userMsg("<conversation_history_summary>\nAll tasks … completed.\n</conversation_history_summary>", at("2026-08-14T10:00:01Z")),
      userMsg('Use the TaskOutput tool with task_id="abc" to retrieve the full output', at("2026-08-14T10:00:02Z")),
      userMsg("<system-reminder>x</system-reminder>\n<user_query>carry on</user_query>", at("2026-08-14T10:00:03Z")),
    ]);
  ...
  it("unwraps a prompt that carries an attached image's local path", () => {
    ...
    expect(read([userMsg(text, at("2026-08-14T10:00:00Z"))])).toEqual([
      { role: "user", content: "@image#1:shot.png what is this?", timestamp: "2026-08-14T10:00:00.000Z" },
    ]);
  });
  ...
  it("ignores non-conversation records, empty renders and malformed lines", () => {
    const lines = [
      { type: "reasoning", timestamp: at("2026-08-14T10:00:00Z"), content: [], rawContent: [{ type: "reasoning_text", text: "thinking" }] },
      ...
      userMsg("<system-reminder>only harness scaffolding</system-reminder>", at("2026-08-14T10:00:05Z")),
      null,
      42,
      "{ not valid json",
    ];
  ...
  it("keeps a tool call whose arguments the model produced malformed", () => {
    const turns = read([{ type: "function_call", name: "Bash", arguments: "{oops", timestamp: at("2026-08-14T10:00:00Z") }]);
    expect(turns).toEqual([
